...
<!-- 星級評論（推薦度） -->
    ...
    <? for($i=1;$i<=5;$i++){
    if($i == $_GET['suggest']){
       echo "<input type='radio' name='rating' class='rating__control' value='$i' id='rc$i' checked>"; 
    }else{
...
<? for($i=6;$i<=10;$i++){
    if($i-5 == $_GET['level']){
        echo "<input type=radio name=rating1 class=rating__control id=rc$i checked>";
    }else{
...
<svg xmlns="http://www.w3.org/2000/svg" style="display: none">
  <symbol id="star" viewBox="0 0 26 28">
    ...
    <textarea class="form-control" id="exampleTextarea" name='exampleTextarea' rows="5"><? echo $_GET['comment']?></textarea>
  </div>
...
fetch(a,{
  headers:{"Authorization": "Bearer keyX6caWycl8lSrpB","Content-Type": "application/json; charset=utf-8" },
    ...
    body: JSON.stringify({
        'fields':{
"Comment_ID":"<? echo $_GET['Comment_ID']?>",
  "Student_ID": [
    "<? echo $_GET['studentid']?>"
  ],
