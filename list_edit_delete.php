           ...
           $.each(Student.records,function(i,item){
               if(item.fields.Student_ID == "<? echo $_SESSION['userid']?>"){
                   $.each(item.fields.Comment ,function(i,item1){
                       ...
                       courseid = Comment.records[Comment_index].fields.Course_ID[0];
                       ...
                       private = Comment.records[Comment_index].fields.private;
                    ...
                    teacherid =Course.records[Course_index].fields.Teacher_ID[0];
                       teachername = Teacher.records[teacher_index].fields.Teacher_name;
                       studentname = item.fields.Student_name;
                       studentid = item.id;
                       grade = item.fields.grade;
                       depart_index = DepartmentidArray.indexOf(item.fields.Major_ID[0]);
                       studentdepart = Department.records[depart_index].fields.Department_name;
                       suggest = Comment.records[Comment_index].fields.Suggestion_rate;
                       level = Comment.records[Comment_index].fields.Lesson_level;
                       comment = Comment.records[Comment_index].fields.Comments;//評論
                       comment_ID = Comment.records[Comment_index].id;//真實的id
                       Comment_ID =Comment.records[Comment_index].fields.Comment_ID;//方便人看的id;
                       console.log(Coursename,teachername,Year,section,studentdepart,grade,suggest,level,comment,comment_ID ,teacherid);
                       plus_test = plus_test+ "<tr><td>"+num+"</td><td>"+Coursename+"</td><td>"+teachername+"</td><td>"+Year+"</td><td>第"+section+"學期</td><td>"+
                       "<a href='commet.php?Coursename="+Coursename+"&teachername="+teachername+"&Year="+Year+"&section="+section+"&studentdepart="+studentdepart+"&grade="+grade+"&suggest="+suggest+"&level="+level+"&comment="+comment+"&comment_ID="+comment_ID+"&studentname="+studentname+"&private="+private+"&courseid="+courseid+"&Comment_ID="+Comment_ID+"&studentid="+studentid+"' class=edit title=Edit data-toggle=tooltip><i class=material-icons>&#xE254;</i></a>"+
                        "<a href=comment_del.php?comment_ID="+comment_ID+" class=delete title=Delete data-toggle=tooltip><i class=material-icons>&#xE872;</i></a></td></tr>"
